// pages/database/components/Toolbar.jsx
import Button from '@/components/Button';

/**
 * Верхняя панель: заголовок, поиск и счётчик результатов.
 * q: { input:string, value:string } — input управляет полем, value — применённый запрос.
 */
  ...
  return (
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      ...
      >
        ...
        />
        <Button type="submit" variant="tab" size="sm" className="px-4">
          Найти
        </Button>
        {q.value ? (
      ...
      <div className="text-sm text-slate-500">{loading ? 'Загрузка…' : `Найдено: ${total}`}</div>
    </div>
  );
}
