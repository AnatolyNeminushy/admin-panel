// pages/analytics/components/SmoothChart.jsx
// Гладкий SVG-график со сглаживанием кривой, area-заливкой и ховером.
// Вход: массив точек [{ dayISO, day, [yKey]: number }]. Автоматически подрезает
// ведущие/замыкающие нули, делает даунсэмплинг и вычисляет «красивые» оси.
...
export default function SmoothChart({ data, yKey = 'sum', height = 220, robustScale = true }) {
  ...
  // 1) Убираем хвосты с нулевыми значениями (для фокуса на реальных периодах)
  const trimmed = (() => {
  ...
  // 2) Даунсэмплинг серии до ~180 точек, чтобы график был лёгким и отзывчивым
  const maxPoints = 180;
  const series = (() => {
  ...
  // 3) Метки по X: не более 10 подписей
  const maxXTicks = 10;
  const step = Math.max(1, Math.ceil(series.length / maxXTicks));

  // 4) Робастное масштабирование Y (по 90-й перцентиле), чтобы пики не «сжимали» шкалу
  ...
  // 6) Геометрия и нормировки координат
  const W = 560;
  const H = height;
  ...
  // 7) Построение сглаженного пути (кривая Безье между соседними точками)
  const pathD = (() => {
    if (series.length === 1) {
      const x = normX(0);
      const y = normY(series[0][yKey]);
      return `M${x} ${y} L${x + 0.01} ${y}`;
    }
    let d = '';
    for (let i = 0; i < series.length; i++) {
      const x = normX(i);
      const y = normY(series[i][yKey]);
      if (i === 0) d += `M${x} ${y}`;
      else {
        const x0 = normX(i - 1);
        const y0 = normY(series[i - 1][yKey]);
        const cx1 = x0 + (x - x0) / 2;
        const cy1 = y0;
        const cx2 = x0 + (x - x0) / 2;
        const cy2 = y;
  ...
  return (
    <div className="w-full">
      <svg
        ...
        role="img"
        aria-label="График значений по дням"
      >
        ...
        {/* Метки оси X (редкие, чтобы не засорять) */}
        {series.map((d, i) => {
          if (i % step !== 0 && i !== series.length - 1) return null;
          const x = normX(i);
          return (
            <text key={`xt-${d.dayISO ?? i}`} x={x} y={H - 8} fontSize="10" textAnchor="middle" fill="#6b7280">
              {d.day}
            </text>
          );
        })}

        {/* Линейный градиент для заливки под кривой */}
        <defs>
        ...
        {hover && (
          <>
            ...
            <text
              x={Math.min(hover.x + 18, W - 120)}
              y={pad.t + 44}
              fontSize="12"
              fill="#fff"
              fontWeight="600"
            >
              {Number(series[hover.i][yKey] || 0).toLocaleString('ru-RU')}
              {yKey === 'sum' ? ' ₽' : ''}
            </text>
          </>
        )}
